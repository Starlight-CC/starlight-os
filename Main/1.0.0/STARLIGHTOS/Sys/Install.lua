...
if nOption  == 1 then
    ...
    local file = http.get(OSLst)
    local fh = fs.open("/temp/OSLst.tab", "w")
    fh.write(file.readAll())
    fh.close()
    dofile("/temp/OSLst.tab")
    ...
    local file = http.get(OSurl)
    local fh = fs.open("/temp/OSurl.tab", "w")
    fh.write(file.readAll())
    fh.close()
    dofile("/temp/OSurl.tab")

    local file = http.get(verurl[nOption])
    local fh = fs.open("/temp/OSF.tab", "w")
    fh.write(file.readAll())
    fh.close()
    dofile("/temp/OSF.tab")
